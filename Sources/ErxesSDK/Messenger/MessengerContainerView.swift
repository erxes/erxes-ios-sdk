...
struct MessengerContainerView: View {
    ...
    var body: some View {
        ...
        Group {
            if #available(iOS 18, *) {
                // iOS 18+: native TabView — on iOS 26 this automatically becomes liquid glass
                nativeTabView(primary: primary)
            } else {
                // iOS 16–17: custom floating material bar
                legacyTabView(primary: primary)
            }
        }
        .tint(primary)
        .environmentObject(appVM)
        .task { await appVM.connect(config: config, user: user) }
    }
    ...
    private func homeInputBar(primary: Color) -> some View {
        HStack(spacing: 12) {
            Text("Welcome to erxes! How can I help you today?")
                .font(.subheadline)
