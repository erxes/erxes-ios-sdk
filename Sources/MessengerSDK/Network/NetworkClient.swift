...
final class NetworkClient {
    ...
    func configure(endpoint: String) {
        // messenger gateway always lives at /gateway/graphql (fileEndpoint, no w. subdomain)
    ...
    var client: ApolloClient {
        guard let apollo else {
            fatalError("NetworkClient not configured — call MessengerSDK.configure() first")
        }
        return apollo
    }
}
